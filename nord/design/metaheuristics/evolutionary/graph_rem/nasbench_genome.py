...
from .rem_config import UNEVALUATED_FITNESS, FULL_CHANNELS_NO, DROPOUT_PROB
...
# FMNIST IJNS paper
layers_list = [CONV1X1, CONV3X3, MAXPOOL3X3]


class Genome(object):

    def __init__(self, identity_rate, add_node_rate,
                 innovation=None):
    ...
    def to_descriptor(self):
        ...
        for g in self.nodes.genes:
            ...
            if gene.innovation_number in actives and gene.enabled:
                if not gene.io:
                    ...
                    # Get the parameters
                    layer, params = layers_list[gene.value[0]], None

                    descriptor.add_layer(layer, params, name=innv)

        # Add IO layers
        descriptor.add_layer('input', {}, name=INPUT_NAME)
        descriptor.add_layer('output', {}, name=OUTPUT_NAME)
        ...
        for g in self.connections.genes:
            ...
            # Connect all active
            if gene.enabled:
                descriptor.connect_layers(str(from_), str(to_))
        return descriptor
