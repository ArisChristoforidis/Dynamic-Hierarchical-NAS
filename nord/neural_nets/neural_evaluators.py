...
from nord.configs import (CHANNELS, CRITERION, DIMENSION_KEEPING, INPUT_SHAPE,
                          NUM_CLASSES, PROBLEM_TYPE, METRIC)
from nord.neural_nets import NeuralDescriptor, NeuralNet, GraphRankNet
...
class AbstractNeuralEvaluator():
    ...
    def train(self, net: nn.Module, epochs: int):
        ...
        criterion_loss = CRITERION[self.dataset]()

        if CRITERION[self.dataset] == nn.KLDivLoss:
    ...
    def net_evaluate(self, net: nn.Module,  epochs: int,
        ...
        # loss, accuracy = 1337, 1337

        if not return_net:
            return loss, accuracy, total_time
        else:
            return loss, accuracy, total_time, net
...
class LocalEvaluator(AbstractNeuralEvaluator):
    ...
    def load_data(self, data_percentage: float, dataset: str):
        ...
        if not self.data_loaded:
            ...
            if dataset == 'cifar10':
                (self.trainloader,
                 self.testloader,
                 self.classes) = get_cifar10(data_percentage)
            elif dataset == 'fashion-mnist':
                (self.trainloader,
                 self.testloader,
                 self.classes) = get_fashion_mnist(data_percentage)

            elif dataset == 'graph-nasbench1':
                from nord.design.metaheuristics.evolutionary.graph_rem.generate_dataset import \
                ...
                (self.trainloader,
                 self.testloader,
                 self.classes) = get_graph_data_loaders(graph_bs,
                                                        initial_pop, mutated, classes, val_perc=0.0,
                                                        test_perc=0.7, all_vs_all=[False, True])

            elif dataset == 'graph-nasbench2':
                from nord.design.metaheuristics.evolutionary.graph_rem.generate_dataset import \
                    get_graph_data_loaders
                classes = 2
                initial_pop = 20
                ...
                (self.trainloader,
                 ...
                 self.classes) = get_graph_data_loaders(graph_bs,
                                                        initial_pop, mutated, classes, val_perc=0.0,
                                                        test_perc=0.3, all_vs_all=False)

            elif dataset == 'graph-nasbench3':
                from nord.design.metaheuristics.evolutionary.graph_rem.generate_dataset import \
                    get_graph_data_loaders
                classes = 3
                initial_pop = 20
    ...
    def descriptor_to_net(self, descriptor: NeuralDescriptor, untrained: bool):
        """Make a net from descriptor, accounting for any distributed comms
        """
        if PROBLEM_TYPE[self.dataset] == 'ranking':
            return GraphRankNet(descriptor, NUM_CLASSES[self.dataset],
        ...
        return NeuralNet(descriptor, NUM_CLASSES[self.dataset],
                         INPUT_SHAPE[self.dataset], CHANNELS[self.dataset],
                         untrained=untrained,
                         keep_dimensions=DIMENSION_KEEPING[self.dataset],
